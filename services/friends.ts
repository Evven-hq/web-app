...
function normalizeActivity(raw: unknown): FriendActivity {
  ...
  return {
    id: asString(
      item.id ??
        item.friend_activity_id ??
        item.expense_id ??
        crypto.randomUUID?.() ??
        "",
    ),
    type: asString(
      item.type ?? item.kind ?? item.entry_type ?? "expense",
    ) as FriendActivity["type"],
    ...
    payment_method: item.payment_method as
      FriendActivity["payment_method"] | null,
    settlement_direction: (item.settlement_direction ??
      item.direction ??
      null) as FriendActivity["settlement_direction"] | null,
    settlement_amount: asNullableAmount(item.settlement_amount),
    direction: asNullableString(
      item.direction,
      item.settlement_direction,
    ) as FriendActivity["direction"],
...
function normalizeFriend(raw: unknown): Friend {
  ...
  return {
    ...
    id: asString(
      friend.group_id ?? friend.id ?? friend.friend_id ?? friend.ghost_id ?? "",
    ),
    name: asString(
      friend.name ?? friend.full_name ?? friend.display_name ?? "Friend",
    ),
    user_code:
      friend.user_code === undefined ? null : asString(friend.user_code),
    profile_picture:
      friend.profile_picture === undefined
        ? null
        : asString(friend.profile_picture),
    shadow_group_id:
      friend.shadow_group_id === undefined
        ? null
        : asString(friend.shadow_group_id),
    balance,
    net_balance: asNullableAmount(friend.net_balance) ?? balance ?? 0,
    status: friend.status === undefined ? null : asString(friend.status),
    last_activity_at:
      friend.last_activity_at === undefined
        ? null
        : asString(friend.last_activity_at),
    created_at:
      friend.created_at === undefined ? undefined : asString(friend.created_at),
    updated_at:
      friend.updated_at === undefined ? undefined : asString(friend.updated_at),
    expenses: history,
...
function normalizeFriendList(payload: unknown): FriendListItem[] {
  ...
  return Array.isArray(rawList)
    ? rawList.map((item) => normalizeFriend(item))
    : [];
}
...
function normalizeFriendRequest(
  ...
  return {
    ...
    name: asString(
      item.name ?? item.user_name ?? item.requester_name ?? "Friend",
    ),
    user_code: item.user_code === undefined ? null : asString(item.user_code),
    profile_picture:
      item.profile_picture === undefined
        ? null
        : asString(item.profile_picture),
    created_by:
      item.created_by === undefined ? null : asString(item.created_by),
    created_by_name:
      item.created_by_name === undefined
        ? null
        : asString(item.created_by_name),
    created_by_user_code:
      item.created_by_user_code === undefined
        ? null
        : asString(item.created_by_user_code),
    created_at:
      item.created_at === undefined ? undefined : asString(item.created_at),
    status: item.status === undefined ? undefined : asString(item.status),
    direction: (item.direction ?? direction ?? null) as
      FriendRequest["direction"] | undefined,
  };
}
...
function normalizeFriendRequests(payload: unknown): FriendRequestsResponse {
  ...
  return {
    incoming: incomingSource.map((item) =>
      normalizeFriendRequest(item, "incoming"),
    ),
    outgoing: outgoingSource.map((item) =>
      normalizeFriendRequest(item, "outgoing"),
    ),
  };
}
...
export function isActiveFriendDetail(
  friend: Friend | FriendRequest | null | undefined,
): friend is FriendDetail {
...
export async function createFriendRequest(
  ...
  if (isPlainObject(data) && ("incoming" in data || "outgoing" in data)) {
    ...
    return (
      requests.incoming[0] ?? requests.outgoing[0] ?? normalizeFriend(data)
    );
  }
...
export function combineFriendHistory(
  friend: FriendDetail | FriendListItem | Friend,
): FriendActivity[] {
  const items = [
    ...(friend.history ?? []),
    ...(friend.expenses ?? []),
  ...
  return items.map(normalizeActivity).sort((a, b) => {
...
export function getFriendBalance(
  friend: Pick<Friend, "balance" | "net_balance">,
) {
